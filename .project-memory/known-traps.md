...
- Area / files: `android/accessibility-companion/app/src/main/AndroidManifest.xml` (`POST_NOTIFICATIONS`), `src/phonectl/companion_setup.py` (`pm grant`), `CompanionForegroundService.kt` (`startForeground`)
- Symptom: on API 33+ (targetSdk 35), the persistent "Stop phonectl" foreground-service notification — one of two user-facing emergency-stop surfaces, the other being the QS tile — never appears if the companion is installed without ever running `phonectl companion setup`, since a manifest `<uses-permission>` alone does not grant a dangerous runtime permission.
- Why: nothing in the Kotlin source calls `ActivityCompat.requestPermissions`/`registerForActivityResult` for `POST_NOTIFICATIONS` (grepped, zero hits). Today it silently works anyway because `companion_setup.py` runs `adb shell pm grant … POST_NOTIFICATIONS`, an ADB-privileged grant that bypasses the runtime dialog entirely.
- Safe approach: this is currently masked by the ADB-based setup flow, so it is not an active bug — but it is a real gap for the project's stated ADB-optional/no-root direction (Phase 7.1 Shizuku and the 2026-07-05 "ADB-free observe" pass point the same way). Before any ADB-less onboarding path ships, add an in-app runtime request for `POST_NOTIFICATIONS` (e.g. from `SettingsActivity.onCreate`), mirroring the existing notification-listener access-hint pattern.
- Status (addressed, code-complete): `SettingsActivity.onCreate` now calls `maybeRequestPostNotifications()` — a `registerForActivityResult(RequestPermission())` launch, gated on API 33+ and a missing grant. The ADB `pm grant` in setup still short-circuits it on the ADB path. **Not yet device-validated** (no hardware in CI); the verification below is still owed on a real API 33+ device.
- Verification: uninstall the companion, install without ever running `phonectl companion setup`, open the app once, then trigger the foreground service — confirm whether the notification actually posts on a real API 33+ device.
