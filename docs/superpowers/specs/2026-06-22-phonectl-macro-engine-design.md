...
- `docs/superpowers/phonectl-platform-roadmap.md` — **§5** (Phase 6 row) and **§5.1** ordering rationale
...
- `docs/superpowers/phonectl-automation-platform-strategy.md` — **§12** (Tasker/MacroDroid-inspired
...
- `docs/superpowers/specs/2026-06-22-phonectl-daemon-event-runtime-design.md` — the daemon spec this builds
...
- `docs/superpowers/plans/2026-06-22-phonectl-remaining-plans-meta-plan.md` — **§3 Phase 6 stub** and the
  authoring rules (brainstorm → spec → plan is mandatory before any Phase 6 plan).
- Plan **2.1** (`runtime.run_action` funnel + audit v2), Plan **2.2** (`risk`/`policy`/`ratelimit`),
  Plan **3.1** (`ProviderRegistry` + `cli.build_runtime`), Plans **3.2–3.5**/**4.x** (the action +
  capability surface a macro's action steps map onto), Plans **5.1/5.2** (daemon, event bus, snapshot
  cache, run records).
