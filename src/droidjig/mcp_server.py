...
def _default_build(cfg):
    from droidjig.cli import build_runtime
...
# engage STOP (phone_stop) but cannot clear it — resume via `droidjig resume` on
...
def _macro_validate_mcp(macro, **_):
    from droidjig.macro import schema as _ms
...
def _macro_run_mcp(macro, *, build, yes=False, **_):
    ...
    from droidjig.cli import macro_fn_for
...
TOOLS = {
    ...
    # human-only, out-of-band action (Finding 1). Resume via `droidjig resume`
...
def serve(build=_default_build) -> None:
    try:
        ...
        raise errors.CapabilityUnavailableError("MCP SDK not installed; pip install droidjig[mcp]") from e
    app = FastMCP("droidjig")
