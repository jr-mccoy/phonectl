...
from droidjig import companion_setup as _companion_setup
...
from droidjig.providers.accessibility import AccessibilityProvider  # noqa: F401
...
from droidjig.daemon import discovery as _daemon_discovery
from droidjig.daemon.client import DaemonClient


def macro_fn_for(step, scopes):
    from droidjig import actuator, errors
    from droidjig.macro import variables as _mvars
...
def _cmd_observe(args):
    ...
    if not env.get("ok"):
        if getattr(args, "json", False):
            ...
            print(f"droidjig: {env['error']['message']}")
        return 1
    if getattr(args, "json", False):
        print(json.dumps(env, indent=2))
    else:
...
def _do_action(args, verb, fn, target):
    ...
    if detach and env.get("ok") and "job_id" in env.get("data", {}):
        print(f"droidjig: job {env['data']['job_id']} (use: droidjig job {env['data']['job_id']})")
        return 0
    if env["ok"]:
        if getattr(args, "json", False):
            print(json.dumps(env, indent=2))
        elif env.get("dry_run"):
            print(f"droidjig: dry-run {verb} {target} (not executed)")
        else:
            _emit(env["data"])
        return 0
    if getattr(args, "json", False):
...
def _cmd_swipe(args):
    ...
    if len(swipe_args) == 4:
        try:
            ...
            print("droidjig: swipe coords must be integers")
            return 2
        return _do_action(args, "swipe",
                          lambda b, s: actuator.swipe(b, s, x1, y1, x2, y2),
                          {"coords": [x1, y1, x2, y2]})
    print("droidjig: swipe requires a direction (up/down/left/right) or x1 y1 x2 y2")
    return 2
...
def _cmd_scroll_until(args):
    ...
    if text is None and sel is None:
        print("droidjig: scroll-until requires --text or --selector")
        return 2
    return _do_action(
        args, "scroll_until",
...
def _cmd_wait_for(args):
    sel = _selector_from_args(args)
    if sel is None:
        print("droidjig: wait-for requires --text, --id, or --selector")
        return 2
    cfg = config.load()
    backend, session, conn = build_runtime(cfg)
    conn.ensure()
    ...
    if snap is None:
        print("droidjig: wait-for timed out")
        return 1
    _emit(snap)
    return 0
    ...
    try:
        if args.port:
            ...
            if backend.get_state() != "device":
                print(f"droidjig: {GUIDANCE}")
                return 1
            print(f"droidjig: reconnected to {args.port}")
            return 0
        addr = conn.rediscover()
        print(f"droidjig: reconnected to {addr}")
        return 0
...
def _cmd_doctor(args):
    ...
    if getattr(args, "bundle", None):
        ...
        print(f"droidjig: diagnostics bundle written to {path}")
        return 0
    try:
        conn.ensure()
    except ConnectionError as e:
    ...
    if getattr(args, "json", False):
        ...
        print(f"droidjig: connected (serial={backend.serial}, state={data['state']})")
    return 0
...
def _cmd_companion_setup(args):
    ...
    if getattr(args, "apk", None) and not os.path.exists(args.apk):
        print(f"droidjig: apk not found: {args.apk}")
        return 2
    apk = _resolve_apk(getattr(args, "apk", None))
    if apk is None:
        print("droidjig: no app-debug.apk found; pass --apk PATH")
        return 2
    ...
    if getattr(args, "json", False):
        ...
        print("droidjig: companion setup " + ("OK" if res["ok"] else "FAILED"))
...
def _cmd_config_set(args):
    ...
    try:
        ...
        return 2
    config.save(cfg)
    print(f"droidjig: {args.key} = {cfg[args.key]}")
    return 0


def _cmd_policy(args):
    if getattr(args, "policy_cmd", None) != "explain":
        print("droidjig: policy requires explain")
        return 2
    cfg = config.load()
    backend, session, conn = build_runtime(cfg)
    conn.ensure()
    ...
    if getattr(args, "json", False):
        ...
        print(f"droidjig: {args.verb} -> {out['decision']} (risk={out['risk_level']})")
    return 0
...
def _cmd_stop(args):
    ...
    if getattr(args, "json", False):
        ...
        print("droidjig: STOP engaged (kill switch active)")
    return 0
...
def _cmd_resume(args):
    ...
    if getattr(args, "json", False):
        ...
        print("droidjig: STOP cleared" if cleared else "droidjig: no STOP was engaged")
    return 0
...
def _cmd_audit(args):
    ...
    if args.audit_cmd == "purge":
        n = audit.purge()
        print(f"droidjig: purged {n} audit record(s)")
        return 0
    if args.audit_cmd == "export":
        path = audit.export(args.path, redacted=not args.no_redact)
        print(f"droidjig: exported audit log to {path}")
        return 0
    print("droidjig: audit requires tail|purge|export")
    return 2
...
def _cmd_get_focused_field(args):
    ...
    if getattr(args, "json", False):
        ...
        print("droidjig: no focused field")
    return 0
...
def _cmd_device_battery(args):
    ...
    if p is None:
        env = results.err(
            ...
            user_action="Install Termux:API and run 'droidjig setup termux-api'.",
        )
        if getattr(args, "json", False):
            print(json.dumps(env, indent=2))
        else:
...
def _cmd_notifications_list(args):
    ...
    if provider is None:
        env = results.err(
            ...
            user_action="Install the droidjig companion APK or Termux:API.",
        )
        if getattr(args, "json", False):
            print(json.dumps(env, indent=2))
        else:
...
def _cmd_notifications_wait(args):
    ...
    if getattr(args, "json", False):
        ...
        print("droidjig: no matching notification found (timed out)")
    return 0
...
def _cmd_notifications_reply(args):
    ...
    if provider is None:
        env = results.err(
            ...
            user_action="Install the droidjig companion APK.",
        )
        if getattr(args, "json", False):
            print(json.dumps(env, indent=2))
        else:
...
def _cmd_macro_validate(args):
    from droidjig.macro import loader, schema
    cfg = config.load()
    if not hasattr(args, "path") or args.path is None:
        print("usage: droidjig macro validate <path>")
        return 1
...
def _cmd_macro_status(args):
    cfg = config.load()
    from droidjig.macro import records as _mrec
...
def _cmd_macro_cancel(args):
    ...
    client = _daemon_client(cfg)
    if client is None:
        print("droidjig: no running daemon (cancel has no effect)")
        return 0
...
def _cmd_macro_enable(args):
    from droidjig.macro import loader, registry
    doc = loader.load(args.path)
    registry.enable(doc)
...
def _cmd_daemon(args):
    ...
    if sub == "start":
        ...
        print(f"droidjig daemon listening on {host}:{port} (Ctrl-C to stop)")
        try:
            srv.serve_forever()
        finally:
            srv.shutdown()
    ...
    if sub == "stop":
        ...
        if client is None:
            ...
            print("droidjig: no running daemon")
            return 0
        client.call("shutdown", {})
        print("droidjig: shutdown signalled")
        return 0
    print("usage: droidjig daemon {start|status|stop}")
    return 1
...
def _cmd_job(args):
    ...
    while True:
        ...
        if not env.get("ok"):
            print(json.dumps(env, indent=2) if getattr(args, "json", False) else f"droidjig: {env['error']['message']}")
            return 1
    ...
    if getattr(args, "json", False):
        ...
        print(f"droidjig: job {args.job_id} status={env['data']['status']}")
    return 0


def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="droidjig")
...
# Exit code for a bug in droidjig itself, distinct from the documented 1/2/3
# (policy denial, kill switch, confirm refusal) so scripts can tell "droidjig
# refused" from "droidjig broke".
INTERNAL_ERROR_EXIT = 4
SIGINT_EXIT = 130   # 128 + SIGINT, the shell convention

_ISSUES_URL = "https://github.com/jr-mccoy/droidjig/issues"
...
def main(argv=None) -> int:
    ...
    try:
        ...
        print("droidjig: interrupted")
        return SIGINT_EXIT
    except BrokenPipeError:
        # `droidjig observe --json | head` closes the pipe on us. Redirect the
        ...
        # not a DroidjigError is a bug in droidjig (or an unhandled OS condition),
        # and users hit those too — so it gets the same structured treatment
        # instead of a stack trace. DROIDJIG_DEBUG=1 re-raises for developers.
        if os.environ.get("DROIDJIG_DEBUG"):
            raise
        env = results.err(
            ("internal_error", f"{type(e).__name__}: {e}"),
            user_action=f"This is a bug in droidjig. Please report it at {_ISSUES_URL} "
                        f"(re-run with DROIDJIG_DEBUG=1 for the traceback).",
        )
        if getattr(args, "json", False):
            print(json.dumps(env, indent=2))
        else:
            print(f"droidjig: internal error — {env['error']['message']}")
            print(f"droidjig: {env['error']['user_action']}")
