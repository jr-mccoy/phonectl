...
import com.droidjig.companion.R
import com.droidjig.companion.state.Capabilities
import com.droidjig.companion.state.SharedPrefsTrustState
...
import com.droidjig.companion.transport.Server

/**
 * Foreground service hosting the loopback NDJSON [Server] and the persistent "Stop droidjig"
...
class CompanionForegroundService : Service() {
    ...
    private fun settingsPendingIntent(): PendingIntent {
        val intent = packageManager.getLaunchIntentForPackage(packageName)
            ?: Intent().setClassName(packageName, "com.droidjig.companion.ui.SettingsActivity")
    ...
    companion object {
        const val CHANNEL_ID = "droidjig_automation"
        const val NOTIF_ID = 0x9C7 // arbitrary stable id

        /** Logcat tag for the transport audit line (method + outcome only — never payloads). */
        const val TRANSPORT_LOG_TAG = "droidjig-companion"

        const val ACTION_START = "com.droidjig.companion.action.START"
        const val ACTION_STOP = "com.droidjig.companion.action.STOP"
        const val ACTION_RESUME = "com.droidjig.companion.action.RESUME"
        const val ACTION_REFRESH = "com.droidjig.companion.action.REFRESH"
